...
profilesRouter.get("/", async (req, res) => {
  try {
    const profilesListData = await connectionPool.query(
      `select p.profile_id, s.status_id, p.name, p.birthdate, p.location, p.city, p.sexident, p.sexprefer, p.racialprefer,p.meetprefer, p.about_me as "aboutMe" from users u inner join user_profiles p on u.user_id = p.user_id inner join user_statuses s on u.user_id = s.user_id`
    );
